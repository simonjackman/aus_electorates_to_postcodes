...
We subset this file to NSW addresses, yielding `r format(nrow(nsw_addresses),big.mark=",")` addresses, spanning `r format(nsw_addresses %>% distinct(postcode) %>% nrow(),big.mark=",")` postcodes and `r format(nsw_geo %>% nrow(),big.mark=",")` distinct geo-codes (latitude/longitude pairs).
...
```{r}
#| eval: false
write.csv(out, file = here("aux_data/nsw/district_postcode_counts.csv"),row.names = FALSE)
```
